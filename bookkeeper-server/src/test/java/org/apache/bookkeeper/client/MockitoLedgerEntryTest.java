...
public class MockitoLedgerEntryTest {
    ...
    @InjectMocks
    ByteBuf mockedBuff = new ByteBuf() {
        @Override
        public int capacity() {
            return 0;
        }
    ...
    public static Collection<Long[]> getParameters(){
        return Arrays.asList(new Long[][]{
                {2L, 1L, 1L}, // expected(ledgerId), entryId, length
                {3L, 2L, 1L}, // expected(ledgerId), entryId, length
                {4L, 3L, 1L}, // expected(ledgerId), entryId, length
        });
    }

    public MockitoLedgerEntryTest(long ledgerId, long entryId, long length) {
        // expected value
    ...
    public void setupMock() {
        mockedEntry = Mockito.mock(LedgerEntryImpl.class);
        when(mockedEntry.getLedgerId()).thenReturn(this.ledgerId);
        when(mockedEntry.getEntryId()).thenReturn(this.entryId);
        when(mockedEntry.getLength()).thenReturn(this.length);
    ...
    public void testGetLedgerId() {
        ...
        long returnedValue = ledgerEntry.getLedgerId();
        Assert.assertEquals(this.ledgerId, returnedValue);
    }
}
